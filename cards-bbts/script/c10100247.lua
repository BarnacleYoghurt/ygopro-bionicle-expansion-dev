...
function c10100247.filter1a(c)
  return c:IsRace(RACE_MACHINE) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c10100247.filter1c,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,c)
end
...
function c10100247.target1(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    return Duel.IsExistingMatchingCard(c10100247.filter1b, tp, LOCATION_DECK+LOCATION_HAND, 0, 1, nil, e, tp, e:GetLabelObject())
  end
  local g=Duel.GetMatchingGroup(c10100247.filter1b, tp, LOCATION_DECK+LOCATION_HAND, 0, nil, e, tp, e:GetLabelObject())
  Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON, g, 1, 0, 0)
end
function c10100247.operation1(e,tp,eg,ep,ev,re,r,rp)
  local g = Duel.SelectMatchingCard(tp, c10100247.filter1b, tp, LOCATION_DECK+LOCATION_HAND, 0, 1, 1, nil, e, tp, e:GetLabelObject())
