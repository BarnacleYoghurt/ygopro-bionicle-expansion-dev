...
function s.filter2a(c,tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsSetCard(0xb08)
end
function s.filter2b(c)
  return c:IsSetCard(0xb08) and c:IsSpellTrap() and not c:IsCode(id)
end
...
function s.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  if chkc then return e:GetLabel()~=2 and chkc:IsSpellTrap() and chkc~=c end
  local b1=Duel.IsExistingTarget(Card.IsSpellTrap,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c)
  local b2=Duel.IsExistingMatchingCard(s.filter2b,tp,LOCATION_DECK,0,1,nil)
  local bkal=eg:Filter(s.filter2a,nil,tp):IsExists(Card.IsSummonType,1,nil,SUMMON_TYPE_XYZ)
...
function s.operation2(e,tp,eg,ep,ev,re,r,rp)
  ...
  if op~=1 then
    if br then Duel.BreakEffect() end --in sequence
