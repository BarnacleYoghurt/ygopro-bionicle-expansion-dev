...
function s.target0(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chkc then s.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc) end
    if chk==0 then return true end
    local e2=e:GetLabelObject():GetLabelObject()
    if Duel.IsMainPhase() and s.cost2(e,tp,eg,ep,ev,re,r,rp,0)
        and s.target2(e,tp,eg,ep,ev,re,r,rp,0) and e2:CheckCountLimit(tp)
...
function s.filter1(c)
    return c:IsSetCard(0xb0a) and c:IsMonster() and c:IsAbleToRemoveAsCost()
end
...
function s.filter2b(c)
    return c:IsSetCard(0xb08) and c:IsMonster()
end
...
function s.operation2(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.SelectMatchingCard(tp,s.filter2c,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,nil)
    if #g>0 then
        ...
        if Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 then
            ...
            aux.DelayedOperation(tc,PHASE_END,id,e,tp,
                ...
                function(_,_,tp) return not Duel.IsTurnPlayer(tp) end,
                RESET_PHASE+PHASE_END+RESET_OPPO_TURN,1,aux.Stringid(id,2)
            )
        end
    end
end